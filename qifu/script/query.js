...
window.searchPhone = function () {
  ...
  fetch(`https://form.gealarm2012.workers.dev/admin-qifu?search=${encodeURIComponent(keyword)}`)
    ...
    .then(data => {
      ...
      console.log("🧪 info.prayers 是数组？", Array.isArray(prayers));
      console.log("🧪 prayers 内容：", prayers);
...
function renderForm(entry) {
  ...
  if (!container) {
    console.error("📛 缺少 #prayersContainer 元素，页面可能未加载完全！");
